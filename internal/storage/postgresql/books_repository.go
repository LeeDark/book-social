...
func (r *BookRepository) ListBooksFiltered(ctx context.Context, filter books.BookFilter) ([]books.Book, error) {
	query := `
		SELECT b.id, b.title, b.slug, b.description
		FROM books b
	`
	...
	if filter.AuthorSlug != "" {
		conditions = append(conditions, `
			EXISTS (
				...
				WHERE ba.book_id = b.id AND a.slug = `+fmt.Sprintf("$%d", len(args)+1)+`
			)
		`)
	...
	if filter.GenreSlug != "" {
		conditions = append(conditions, `
			EXISTS (
				...
				WHERE bg.book_id = b.id AND g.slug = `+fmt.Sprintf("$%d", len(args)+1)+`
			)
		`)
...
func (r *BookRepository) GetBookBySlug(ctx context.Context, slug string) (books.Book, error) {
	...
	var book books.Book
	var description sql.NullString
	if err := r.db.QueryRowContext(ctx, query, slug).Scan(&book.ID, &book.Title, &book.Slug, &description); err != nil {
...
func (r *BookRepository) hydrateBookRelationships(ctx context.Context, bookList []books.Book) error {
	...
	return r.hydrateBookGenres(ctx, bookList, bookIndexes, placeholders, args)
}
...
func (r *BookRepository) hydrateBookAuthors(
	...
	query := `
		SELECT ba.book_id, a.id, a.first_name, a.second_name, a.sur_name, a.slug, a.description
		FROM book_authors ba
		JOIN authors a ON a.id = ba.author_id
	...
	for rows.Next() {
		...
		if err := rows.Scan(&bookID, &author.ID, &author.FirstName, &secondName, &surName, &author.Slug, &description); err != nil {
...
func (r *BookRepository) hydrateBookGenres(
	...
	query := `
		SELECT bg.book_id, g.id, g.name, g.slug, g.description
		FROM book_genres bg
		JOIN genres g ON g.id = bg.genre_id
...
func (r *BookRepository) hydrateBookCovers(ctx context.Context, bookList []books.Book) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&bookID, &cover.ID, &cover.Variant, &cover.URL, &mimeType, &byteSize, &width, &height, &checksum); err != nil {
...
func (r *BookRepository) GetAuthorBySlug(ctx context.Context, slug string) (books.Author, error) {
	const query = `
		SELECT id, first_name, second_name, sur_name, slug, description
		FROM authors
		WHERE slug = $1
		LIMIT 1;
	`
	var author books.Author
	var secondName, surName, description sql.NullString
	if err := r.db.QueryRowContext(ctx, query, slug).Scan(&author.ID, &author.FirstName, &secondName, &surName, &author.Slug, &description); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
